...
bin_width = 0.20
E_max = 30
Ex_min = 10 # Lower limit for emitted gamma energy [MeV].
...
for radiation_type in ["M1"]:#,"E1"]: # Loop over M1 and E1 if you have calculated both
    for i in range(len(inputfiles)):
        ...
        gsf = smutil.strength_function_average(
            ...
            Jpi_list,
            bin_width,
            Ex_min, # Lower limit for emitted gamma energy [MeV].
            Ex_max, # Upper limit for emitted gamma energy [MeV].
            multipole_type = radiation_type
        )
        ...
        # -- rho:
        if radiation_type == "M1":
            """
            Only do this once, not for both radiation types.
            """
            ax_rho.step(
                bins,
                np.append(0, rho),
        ...
        # -- gSF:
        ax_gsf.plot(
            bins_middle[0:len(gsf)],
            gsf,
            label = name + r" ${:s}\,\,$".format(radiation_type),
            linewidth = lw,
            linestyle = linestyle,
            dashes = dashes
        )
