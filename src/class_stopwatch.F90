module class_stopwatch
  ...
  public :: stopwatch, start_stopwatch, stop_stopwatch, reset_stopwatch, &
       ...
       my_mpi_finalize, sum_time_ptn, time_wait, time_tmp, &
  ...
  type(stopwatch), save :: time_total, time_operate, &
       time_restart, time_dump, time_diag, time_wait, time_tmp, time_preproc, &
...
!    !$ end if
  ...
  subroutine print_summary_stopwatch()
    ...
    r = time_total%time - time_preproc%time - time_operate%time &
         - time_mpi_init%time - time_mpi_fin%time &
         - time_orth%time - time_restart%time - time_dump%time
...
#ifdef MPI
    call mpi_allgather(n, 1, mpi_integer, nr, 1, mpi_integer, mpi_comm_world, ierr)
    ntotal = sum(nr)
    displs(1) = 0
    do i = 1, nprocs-1
