...
def check_parity(fn_snt, nf, nparity):
    # Returns true if parity nparity has any levels in current nucleus
    fp = open(fn_snt, 'r')
    ...
    for i in range(sum(n_jorb)):
        ...
        if i+1!=arr[0]: raise Exception("read error")
        norb.append(arr[1])
        lorb.append(arr[2])
        jorb.append(arr[3])
        itorb.append(arr[4])
...
def main(fn_snt, fn_ptn, nf, nparity, truncation):
    fp = open(fn_snt, 'r')
    n_jorb, n_core = [0,0], [0,0]
    n_jorb[0], n_jorb[1], n_core[0], n_core[1]  = read_comment_skip(fp)
    norb, lorb, jorb, itorb = [], [], [], []
    ...
    # if not ans: ans = '0'
    # tmod = int(ans)
    # if tmod > 3 or tmod < 0: raise Exception('input out of range'
    # if tmod == 2 or tmod == 3: 
    ...
    # if tmod == 1 or tmod == 3:
        ...
        #     if len(ans) != 2: raise Exception('read error'
        #     t_list.append( [int(i) for i in ans] )
    orb_list, t_list = truncation[0], truncation[1]
...
if __name__ == "__main__":
    if len(sys.argv)<3: 
        ...
        sys.exit(1)
    import os.path
    if os.path.exists(sys.argv[2]): raise Exception("partition file exists")
    fn_snt = sys.argv[1]
    fn_out = sys.argv[2]
    ...
    if nparity != 1 and nparity != -1: raise Exception("parity error")
